class Bullet(object):
    def __init__(self, x, y, wigth, height, direction, color, step):
        self.x = x
        self.y = y
        self.wigth = wigth
        self.height = height
...
class Enemy(object):
    ...
    def check_x_border_right(self, distance, x_border):
        return self.x + self.width + distance <= x_border

    def move_right(self, distance):
        self.x += distance

    def check_x_border_left(self, distance, x_border):
        return self.x - distance >= x_border

    def move_left(self, distance):
        self.x -= distance

    def check_player_border(self, distance, player_border):
        return self.y + self.height + distance <= player_border
    ...
    def __init__(self, x_start, y_start, x_count, y_count, x_jump, y_jump,
        ...
        for y in range(y_count):
            ...
            for x in range(x_count):
                line.append(Enemy(x_start+x*(x_jump+enemy_width), y_start+y*(y_jump+enemy_height),
                                  enemy_width, enemy_height, enemy_color))
